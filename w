...
if getgenv().CrewCheck == true then
    while true do
...
if getgenv().GunFOV == true then
    game:GetService('Players').LocalPlayer.Character.ChildAdded:Connect(function(gun)
        local name = gun.Name
        if fovs[name] then
            Aiming.FOV.Radius = fovs[name]
        end
    end)
end
...
circle.Filled = false
MainFOV.Circle = circle

-- // Update
function MainFOV.FOV()
    ...
    circle.Visible = Aiming.MainVisible
    circle.Radius = (Aiming.MainFOV * 3)
        ...
        if (Aiming.IsIgnored(Player) == false and Character) then
            ...
            if (HitPartsTemp and Aiming.CheckHealth(Player)) then
                ...
                if (circle.Radius > Magnitude and Magnitude < ShortestDistance) then
                    -- // Check if Visible
                    if (Aiming.VisibleCheck and not Aiming.IsPartVisible(HitPartsTemp, Character)) then end
